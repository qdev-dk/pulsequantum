...
class SequenceBuilder(BagOfBeans):
    ...
    def __init__(self,name:str,number_read_freqs:int = 1,**kwargs):
        ...
        self.add_parameter('delay_time_end',
                      label='Delay time end',
                      unit='s',
                      set_cmd= lambda x : x,
                      vals=vals.Numbers(0,1e9))
    ...
    def sweep_pulse(self):
        ...
        delay_time_end = self.delay_time_end.get()
        fast_time = self.fast_time.get()
        total_time = fast_time + delay_time + delay_time_end
        ...
        nr_delay_segs = int(bool(delay_time*awg_sr >= 1.5)) + int(bool(delay_time_end*awg_sr >= 1.5)) 
        if delay_time*awg_sr >= 1.5:
            for i, v in enumerate(steps):
                seg_ramp.insertSegment((nr_delay_segs+1)*i, ramp, (0, 0), dur=delay_time)
                seg_ramp.insertSegment((nr_delay_segs+1)*i+1, ramp, (-delta_fast, delta_fast), dur=fast_time)
                if delay_time_end*awg_sr >= 1.5:
            ...
            for i, v in enumerate(steps):
                seg_ramp.insertSegment((nr_delay_segs+1)*i, ramp, (-delta_fast, delta_fast), dur=fast_time)
                if delay_time_end*awg_sr >= 1.5:
                    seg_ramp.insertSegment((nr_delay_segs+1)*i+2, ramp, (0, 0), dur=delay_time_end)
                else:
                    self.delay_time_end.set(0)
