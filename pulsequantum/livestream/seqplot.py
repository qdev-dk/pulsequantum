...
def timeunitscaling(max_time:float) -> Tuple[float, str]:
    timescaling, timeunit = getSIScalingAndPrefix((0, max_time))
    return (timescaling, timeunit+'s')
...
def chan_min_max(seq, chans, seqlen):
    minf: float = -np.inf
    inf: float = np.inf
...
def plotter(obj_to_plot: BBObject, **forger_kwargs) -> None:
    ...
    chanminmax = chan_min_max(seq, chans, seqlen)
    ...
    for chanind, chan in enumerate(chans):
        ...
        for pos in range(seqlen):
            ...
            timescaling, timeunit = timeunitscaling(time.max())
