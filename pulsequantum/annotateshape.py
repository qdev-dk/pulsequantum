...
def annotateshape(plotid,x1,y1,elem,chx,chy):
    '''
    ...
    ax.set_title('#Pulse sequence on data {}'.format(plotid),size=20)
    cbaxes = cbaxes[0]
    cbaxes.set_label(f'{s[names[2]].label} ({s[names[2]].unit})',size=20)
    cbaxes.ax.tick_params(labelsize=20)
    divx=11.5
    divy=11.75
    ...
    color=['yellow','red','green','lightblue','violet','pink']
    ...
    for n in range(len(x2)):
        ...
        if x2[n]!=0 or y2[n]!=0:
            ax.plot(x1+COGx-x2[n],y1+COGy-y2[n],marker='o',markersize=10,color=f'{color[n]}',label=f'pulse {round(x2[n]*1000,2)},{round(y2[n]*1000,2)} mV')   
